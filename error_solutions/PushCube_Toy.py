...
def add_perturbation(task_stage, pose, gripper_state=None, perturbation_type="position_offset"):
    ...
    if task_stage == "reach":
        ...
        direction_desc = [f"{d}{p:.3f}" for d, p in zip(directions, perturbation)]
        error_description["direction_description"] = ", ".join(direction_desc)
        # 示例输出："x-0.063, y0.052, z-0.058"
    return pose, gripper_state, error_description
def solve_with_errors(env: PushCubeEnv_toy,log_file, error_stage, perturbation_type="position_offset", seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    logs = []
    success_level = ""
...
if __name__ == '__main__':
    # quickstart
    env = gym.make(
        "PushCube-toy", # there are more tasks e.g. "PushCube-v1", "PegInsertionSide-v1", ...
        num_envs=1,
    ...
    for seed in range(10):
            ...
            # res = solve_pushcube_toy(env,seed=seed,vis=True)
            time.sleep(1)
    print(res[-1])
    env.close()