...
class StackCubeEnv_appleplate(BaseEnv):
    """
    替换成苹果放在盘子上并调用iTHORSceneBuilder, seed([18])以及随机打光
    """
...
class StackCubeEnv_can(BaseEnv):
    """
    替换成罐子放在另一个罐子上并调用并调用replicacad数据集seed(1)以及随机打光
    """
...
class StackCubeEnv_lego(BaseEnv):
    """
    替换成乐高积木放在魔方上并调用并调用replicacad数据集seed(2)以及随机打光
    """
...
class StackCubeEnv_orangebowl(BaseEnv):
    """
    替换成橘子放在碗上并调用ArchitecTHORSceneBuilder, seed([2])以及随机打光
    """
...
class StackCubeEnv_dicebrick(BaseEnv):
    """
    替换成橘子放在碗上并调用ArchitecTHORSceneBuilder, seed([5])以及随机打光
    """
