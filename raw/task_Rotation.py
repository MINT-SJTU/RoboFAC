...
def create_rotating_disk(
    ...
    # ----------------------
    # 3. 关节属性：revolute (转动关节)
    # ----------------------
    disk.set_joint_properties(
        type="revolute",
        limits=[[-np.inf, np.inf]],  # 允许无限旋转
...
@register_env("Rotation-v1", max_episode_steps=200)
class RotationEnv(BaseEnv):
    """
    Task Description
    ----------------
    ...
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([0.4, -0.6, 0.3], [0.15, 0.0, 0.15])
    ...
    def _load_scene(self, options: dict):
        ...
        self.goal = actors.build_red_white_target(
            self.scene,
            radius=0.1,
            thickness=1e-5,
            name="goal_region",
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            cube_radius = self._generate_separated_y_positions(b, 2, (0.1, 0.28), 0.05)
            ...
            self.goal.set_pose(Pose.create_from_pq(p=torch.tensor([-0.1, -0.1, 0]), q=euler2quat(0, np.pi / 2, 0)))
...
@register_env("Rotation-gen1", max_episode_steps=200)
class RotationEnv_gen1(BaseEnv):
    ...
    @property
    def _default_sensor_configs(self):
        pose = sapien_utils.look_at(eye=[-0.4, 0, 0.8], target=[-0.1, 0, 0.1])
        return [CameraConfig("base_camera", pose, 128, 128, np.pi / 2, 0.01, 100)]
    ...
    def _load_scene(self, options: dict):
        ...
        builder.inital_pose = sapien.Pose(p=[0, 0, 0.5])
        self.cubeB = builder.build(name="lemon")
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            cube_radius = self._generate_separated_y_positions(b, 2, (0.1, 0.28), 0.08)
            self.cube_r = cube_radius[:, 1]
            cube_angle = random.uniform(0, 2 * np.pi)
            cube_x = self.disk_x + np.cos(cube_angle) * self.cube_r
            cube_y = self.disk_y + np.sin(cube_angle) * self.cube_r
...
@register_env("Rotation-gen2", max_episode_steps=200)
class RotationEnv_gen2(BaseEnv):
