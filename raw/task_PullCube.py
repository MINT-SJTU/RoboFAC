...
@register_env("PullCube-lego", max_episode_steps=50)
class PullCubeEnv_lego(BaseEnv):
    """
    替换拉玩具飞机并调用iTHORSceneBuilder, seed([8])以及随机打光
    """
    ...
    def _default_human_render_camera_configs(self):
        pose = look_at([-0.4, -0.8, 0.6], [0.0, 0.0, 0.35])
...
@register_env("PullCube-scissor", max_episode_steps=50)
class PullCubeEnv_scissor(BaseEnv):
    """
    替换拉剪刀并调用iTHORSceneBuilder, seed([18])以及随机打光
    """
    SUPPORTED_ROBOTS = ["panda", "fetch"]
    agent: Union[Panda, Fetch]
    goal_radius = 0.1
...
class PullCubeEnv_block(BaseEnv):
    """
    替换拉剪刀并调用replicacad, seed(0)以及随机打光
    """
    SUPPORTED_ROBOTS = ["panda", "fetch"]
    agent: Union[Panda, Fetch]
    goal_radius = 0.1
