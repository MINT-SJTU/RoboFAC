...
class PullCubeToolEnv_golf(BaseEnv):
    """
    替换拉高尔夫球并调用replicacad, seed(1)以及随机打光
    """
...
@register_env("PullCubeTool-dice", max_episode_steps=100)
class PullCubeToolEnv_dice(BaseEnv):
    """
    替换拉骰子并调用replicacad, seed(2)以及随机打光
    """
...
@register_env("PullCubeTool-can", max_episode_steps=100)
class PullCubeToolEnv_can(BaseEnv):
    """
    替换拉番茄汤罐子并调用replicacad, seed(4)以及随机打光
    """
...
@register_env("PullCubeTool-peach", max_episode_steps=100)
class PullCubeToolEnv_peach(BaseEnv):
    """
    替换拉桃子并调用ArchitecTHORSceneBuilder, seed([2])以及随机打光
    """
...
@register_env("PullCubeTool-marble", max_episode_steps=100)
class PullCubeToolEnv_marble(BaseEnv):
    """
    替换拉(可能是大理石弹珠)并调用ArchitecTHORSceneBuilder, seed([5])以及随机打光
    """
