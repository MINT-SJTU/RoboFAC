...
from mani_skill.utils.scene_builder.table import TableSceneBuilder, noTableSceneBuilder
...
@register_env("PickCube-apple", max_episode_steps=50)
class PickCubeEnv_apple(BaseEnv):
    """
    将方块替换成苹果并调用replicacad数据集seed(1)以及随机打光
    """
    ...
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([0.4, -0.4, 0.6], [0.0, 0.0, 0.35])
    ...
    def _load_scene(self, options: dict):
        ...
        self.cube = builder.build(name="apple")
...
@register_env("PickCube-can", max_episode_steps=50)
class PickCubeEnv_can(BaseEnv):
    """
    将方块替换成罐子并调用replicacad数据集seed(2)以及随机打光
    """
...
class PickCubeEnv_ball(BaseEnv):
    """
    将方块替换成网球并调用ai2thor数据集ArchitecTHORSceneBuilder, seed([2])以及随机打光
    """
...
@register_env("PickCube-rubiks", max_episode_steps=50)
class PickCubeEnv_rubiks(BaseEnv):
    """
    将方块替换成魔方并调用ai2thor数据集ArchitecTHORSceneBuilder, seed([5])以及随机打光
    """
...
@register_env("PickCube-lego", max_episode_steps=50)
class PickCubeEnv_lego(BaseEnv):
    """
    将方块替换成一块乐高积木并调用ai2thor数据集并调用iTHORSceneBuilder, seed([0])以及随机打光
    """
...
@register_env("PickCube-lock", max_episode_steps=50)
class PickCubeEnv_lock(BaseEnv):
    """
    将方块替换成锁并调用ai2thor数据集并调用iTHORSceneBuilder, seed([8])以及随机打光
    """
    ...
    @property
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([0.4, -0.8, 0.6], [0.0, 0.0, 0.35])
        return CameraConfig("render_camera", pose, 512, 512, 1, 0.01, 100)
