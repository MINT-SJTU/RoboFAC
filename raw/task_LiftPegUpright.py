...
class LiftPegUprightEnv_box(BaseEnv):
    """
    替换为竖直纸盒并调用iTHORSceneBuilder, seed([0])以及随机打光
    """
    ...
    def evaluate(self):
        ...
        is_peg_upright = (
            torch.abs(torch.abs(euler[:, 2]) - np.pi / 2) < 0.1
        )  # 0.08 radians of difference permitted
        # print(torch.abs(torch.abs(euler[:, 2]) - np.pi / 2))
        close_to_table = torch.abs(self.peg.pose.p[:, 2] - self.peg_half_length) < 0.2
        return {
...
@register_env("LiftPegUpright-can", max_episode_steps=50)
class LiftPegUprightEnv_can(BaseEnv):
    """
    替换为竖直罐头并调用iTHORSceneBuilder, seed([8])以及随机打光
    """
    ...
    def evaluate(self):
        ...
        close_to_table = torch.abs(self.peg.pose.p[:, 2] - self.peg_half_length) < 0.15
        return {
            "success": is_peg_upright & close_to_table,
        }
...
@register_env("LiftPegUpright-cup", max_episode_steps=50)
class LiftPegUprightEnv_cup(BaseEnv):
    """
    替换为竖直杯子并调用iTHORSceneBuilder, seed([18])以及随机打光
    """
    ...
    def evaluate(self):
        ...
        euler = rotation_conversions.matrix_to_euler_angles(qmat, "XYZ")
        is_peg_upright = (
            torch.abs(torch.abs(euler[:, 1])) < 0.1
        )  # 0.08 radians of difference permitted
        # print(torch.abs(torch.abs(euler[:, 1])))
        close_to_table = torch.abs(self.peg.pose.p[:, 2] - self.peg_half_length) < 0.15
        return {
            "success": is_peg_upright & close_to_table,
        }
