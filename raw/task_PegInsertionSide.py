...
@register_env("PegInsertionSide-screwdriver", max_episode_steps=100)
class PegInsertionSideEnv_screwdriver(BaseEnv):
    ...
    #         self.table_scene = noTableSceneBuilder(self)
        ...
        with torch.device(self.device):
            ...
            for i in range(self.num_envs):
                ...
                builder = actors.get_actor_builder(
                    self.scene,
                    id=f"ycb:{'044_flat_screwdriver'}",
                )
                builder.initial_pose = sapien.Pose(p=[0, 0, 0.5])  # 设置螺丝刀的位置
                screwdriver = builder.build(name=f"screwdriver_{i}")  # 构建螺丝刀
                self.remove_from_state_dict_registry(screwdriver)  # 从状态字典中移除旧的物体
                ...
                screwdrivers.append(screwdriver)
                boxes.append(box)

            # 将所有螺丝刀和箱子合并
            self.peg = Actor.merge(screwdrivers, "screwdriver")
            self.box = Actor.merge(boxes, "box_with_hole")
