...
class ToolsTaskEnv(BaseEnv):
    ...
    def _build_circular_tool(self, radius, handle_length, width, height):
        ...
        # 创建材质
        mat = sapien.render.RenderMaterial()
        mat.set_base_color([0, 0, 1, 1])  # 颜色：蓝色
        mat.metallic = 1.0
        mat.roughness = 0.0
        mat.specular = 1.0
        ...
        builder.add_cylinder_collision(
            sapien.Pose([0, handle_length + 0.03, 0]),  # 圆盘中心在原点
            radius=radius,
            half_length=height / 2,  # 近似圆盘的厚度
            density=500,
        )
    ...
    def _build_y_shaped_tool(self, handle_length, branch_length, width, height):
        ...
        # 创建材质
        mat = sapien.render.RenderMaterial()
        mat.set_base_color([0, 1, 0, 1])  # 颜色：绿色
        mat.metallic = 1.0
        mat.roughness = 0.0
        mat.specular = 1.0
        ...
        # 计算分支的连接点
        branch_offset = handle_length / 2 - 2 * (branch_length / 2) * 0.707  # 使得分支从手柄顶部延伸
        branch_spacing = width / 2  # 减小两个分支之间的距离
